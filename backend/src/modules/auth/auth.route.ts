...
import { getLoggedInUserController, loginUserController, logOutAllController, logOutController, refreshTokenController, registerUserController } from "./auth.controller.js";
...
router.route("/logout").post(verifyUser,validate(logoutUserSchema), logOutController)
router.route("/logout-all-devices").post(verifyUser, logOutAllController)
router.route("/refresh-token").post(validate(refreshTokenSchema),refreshTokenController)
